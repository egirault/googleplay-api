#!/usr/bin/python
#
# This script analyzes an APK and tries to recover its .proto file, assuming the APK is using Micro-Protobuf.
# It has only been tested on Google Play Android client (sha1: 0f214c312f9800b01e2a5a7b9766dc880efda110)
...
def index_basic_blocks(dvm, vma, cn):
  ...
  if(len(l) != 1):
    return {} # TODO
    # raise Exception("Unable to find a basic block ending with a sparse-switch in mergeFrom() method of class %s" % cn)    # TODO handle packed-switch (cf 1ere classe dans proto_class_names)
  ss = l[0]

  # Get the offset of the sparse-switch, and the sparse-switch-payload instruction
...
def analyse_bb(bb, k, cn):
  ...
  # Index all invoke-virtual instructions. There should be 2 per basic block; one for reading from the stream, the other for setting the appropriate class member.
